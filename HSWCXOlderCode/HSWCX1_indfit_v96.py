...
out_dir='/home/ringuette/halosat_sourceanalysis/HSWCX_2020Analysis/XspecOutput_'+clean_type+'_v2/'
codever = 'HSWCX1v96a'
...
halo_kT, halo_N = 0.224, 0.12  #from target 135 analyzed with planck_zhu nH
...
file_header='Target,LHB_N,'+nH_type.split('_')[1]+',nH_TTS' #all other headers not hard-coded
file_format='\n{0[0]},{0[1]:.6f},{0[2]:.6f},{0[3]:.3f}'
...
elif nH_type=='_nHtest4': nh = np.repeat(0.4,len(hsIDs))
...
for key in key_list:
    ...
    OVII_LU = str('{:.3f}'.format(float(line_ints[key]['OVII_LU'])*0.035))
    OVII_ratio = str('{:.3f}'.format(float(line_ints[key]['O VIIb'])/float(line_ints[key]['O VIIa'])))
    OVIII_ratio = str('{:.3f}'.format(float(line_ints[key]['O VIIIb'])/float(line_ints[key]['O VIIIa'])))
    NeIX_ratio = str('{:.3f}'.format(float(line_ints[key]['Ne IXb'])/float(line_ints[key]['Ne IXa'])))
    
    #absolute line ratios (relative to O VII)
    OVIII_Oratio = str('{:.3f}'.format(float(line_ints[key]['O VIIIa'])))
    NeIX_Oratio = str('{:.3f}'.format(float(line_ints[key]['Ne IXa'])))

    #scan through nH values
    nhTTS_list=np.linspace(0.,0.24,25)
    nhTTS_list=np.append(nhTTS_list,nh_target)
    for i in range(len(nhTTS_list)):  #loop through nh values up to galactic for HSWCX1
        prefix=codever+'_nh'+str(i)
        nh_TTS = nhTTS_list[i]
        ...
        halo_arr=[key,LHB_target,nh_target,nh_TTS]
        ...
        for i in range(len(spec_list)): #fit model to each spectrum linked between detectors    
            ...
            if (i==0):    
                xray_par[i]={1:str(nh_target)+',-1',\
                              2:str(halo_kT)+',-1',3:'0.3,-1',4:'0,-1',5:'0.,-1',\
                              6:str(CXB_PI)+',-1',7:str(CXB_N)+',-1',\
                              8:'0.097,-1',9:'1.,-1',10:'0.,-1',11:str(LHB_target)+',-1'}
                SWCX_par[i]={1:line_ints['Energy']['LowE']+',-1',2:'0.001,-1',3:'0.,-1',\
                              4:line_ints['Energy']['O VIIa']+',-1',5:'0.001,-1',6:OVII_LU+',-1',\
                              7:line_ints['Energy']['O VIIb']+',-1',8:'0.001,-1',9:'=SWCX:p6*'+OVII_ratio,\
                              10:line_ints['Energy']['O VIIIa']+',-1',11:'0.001,-1',12:'=SWCX:p6*'+OVIII_Oratio,\
                              13:line_ints['Energy']['O VIIIb']+',-1',14:'0.001,-1',15:'=SWCX:p12*'+OVIII_ratio,\
                              16:line_ints['Energy']['Ne IXa']+',-1',17:'0.001,-1',18:'=SWCX:p6*'+NeIX_Oratio,\
                              19:line_ints['Energy']['Ne IXb']+',-1',20:'0.001,-1',21:'=SWCX:p18*'+NeIX_ratio}
                TTS_par[i]={1:str(nh_TTS)+',-1',
                              2:'0.5,-1',3:'1.,-1',4:'0,-1',5:'0.1,0.1,0.,0.,0.5,1.',
                              6:'2.07,-1',7:'1.,-1',8:'0,-1',9:'=TTS:p5'}
            else: 
                xray_par[i]={1:'=Xray:p1',2:'=Xray:p2',3:'=Xray:p3',4:'0,-1',5:'=Xray:p5',\
                              6:'=Xray:p6',7:'=Xray:p7',\
                              8:'=Xray:p8',9:'=Xray:p9',10:'0.,-1',11:'=Xray:p11'}
                ...
                TTS_par[i]={1:'=TTS:p1',2:'=TTS:p2',3:'=TTS:p3',4:'=TTS:p4',5:'=TTS:p5',
                              6:'=TTS:p6',7:'=TTS:p7',8:'=TTS:p8',9:'=TTS:p9'}
                              
        #set dictionary of component names for free parameters. change if model changes
        xray_names={2:'Halo_kT',5:'Halo_N'}  #for O lines, det# is added later
        SWCX_names={3:'LowE',6:'O_VII',12:'O_VIII',18:'Ne_IX'}
        TTS_names={1:'TTS_nH',5:'TTS_N'}
        
        #set Xspec model and parameters
        bkg_model=xs.Model('pow','BKG',1)  #particle background
        xray_model=xs.Model('tbabs*(apec+pow)+apec','Xray',2)   #halo+CXB+LHB
        SWCX_model=xs.Model('gauss+gauss+gauss+gauss+gauss+gauss+gauss','SWCX',3)
        TTS_model=xs.Model('tbabs*(apec+apec)','TTS',4)  #hot thermal component from TTS
        ...
        for i, j in itertools.product(range(len(spec_list)), range(xmodel[i].nParameters)):
            if not ((xmodel[i](j+1).frozen)or(len(xmodel[i](j+1).link)>0)):
                ...
                if (xmodel[i](j+1).index==2):  #halo kT
                    ...
                    xs.Fit.steppar('best Xray:'+str(parnum)+' 0.15 0.30 15')
                elif (xmodel[i](j+1).index==5):  #halo N
                    print 'steppar: Xray', parnum
                    xs.Fit.steppar('best Xray:'+str(parnum)+' 0. 0.2 15')                      
        ...
        for i, j in itertools.product(range(len(spec_list)), range(smodel[i].nParameters)):
            if not ((smodel[i](j+1).frozen)or(len(smodel[i](j+1).link)>0)):
                ...
                xs.Fit.steppar('best SWCX:'+str(parnum)+' 0. 0.02 15') #up to ~0.6 LU, max predicted
        ...
        for i, j in itertools.product(range(len(spec_list)), range(tmodel[i].nParameters)):
            if not ((tmodel[i](j+1).frozen)or(len(tmodel[i](j+1).link)>0)):
                ...
                if (tmodel[i](j+1).index==1):
                    ...
                    xs.Fit.steppar('best TTS:'+str(parnum)+' 0.0 0.248 25')  #abs
                elif (tmodel[i](j+1).index==5):
                    print 'steppar: TTS', parnum
                    xs.Fit.steppar('best TTS:'+str(parnum)+' 0.05 0.5 15')  # norm 
        ...
        if nh_TTS==0.: #initialize csv file header
            file_header+=',X^2,dof,red_X^2'
            file_format+=',{0['+str(csv_num)+']:.6f},{0['+str(csv_num+1)+']},{0['\
                +str(csv_num+2)+']:.6f}'   
            chi_file=open(out_dir+codever+model_note+'.csv','w') #initialize output file
            chi_file.write(file_header)
            chi_file.close()        
            
        
        #output results to csv file spec_dir+run_type
        with open(out_dir+codever+model_note+'.csv','a') as chi_file: 
