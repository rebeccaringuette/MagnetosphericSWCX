...
halo_kT1, halo_N1 = 0.270, 0.678  #v116a results  
halo_kT2, halo_N2 = 0.992, 0.258  #v116a results
#halo_kT1, halo_N1 = 0.270, 0.680  #v116 results  
#halo_kT2, halo_N2 = 0.993, 0.259  #v116 results
...
NeIX_ratio = '0.100'
#LowE_ratios = ['0.','0.46732','0.93463','1.40195','1.86926']  #0%, 50%, 100%, 150%, 200%
LowE_ratios = ['0.925']  #100% He ratio
...
#            'Nov2019+','Nov2019','Dec2019+','Dec2019','Jan2020+','Feb2020+','Mar2020+','GOOD']  #total list
#key_list = ['Mar2020+','GOOD']
key_list = ['Oct2018b+','Nov2018+','Oct2019','Nov2019+','Nov2019','Dec2019+','Dec2019','GOOD']  #good list only
#key_list = ['Dec2019+']
#key_list=['GOOD']
#calculated line ratios are only done for good list, average line ratios used otherwise

for N in range(len(LowE_ratios)):
    LowE_ratio = LowE_ratios[N]  #string
    LowE_type = '_'+str(N)
    ...
    for key in key_list:
        ...
        for i in range(len(spec_list)): #fit model to each spectrum linked between detectors    
            ...
            if (i==0):    
                ...
                SWCX_par[i] = {1:line_ints['Energy']['LowE']+',-1',2:'0.001,-1',3:'=SWCX:p6*'+LowE_ratio,\
        ...
        logFile = xs.Xset.openLog(out_dir+prefix+model_note+LowE_type+'_'+key+'_xspeclog.txt')
        xs.Xset.show()
        xs.AllData.show()  
        ...
        while error_check>0:
            ...
            error_loop+=1
            logFile = xs.Xset.openLog(out_dir+prefix+model_note+LowE_type+'_'+key+'_xspeclog'+str(error_loop)+'.txt')
            xs.Xset.show()
            xs.AllData.show()  
            if (xs.Fit.statistic/xs.Fit.dof<2.):
                #run error on all free BKG parameters
        ...
        if key==key_list[0]: #initialize csv file header
            ...
            file_format+=',{0['+str(csv_num)+']:.6f},{0['+str(csv_num+1)+']},{0['\
                +str(csv_num+2)+']:.6f}'   
            chi_file=open(out_dir+prefix+model_note+LowE_type+'.csv','w') #initialize output file
            chi_file.write(file_header)
            chi_file.close()        
            
        
        #output results to csv file spec_dir+run_type
        with open(out_dir+prefix+model_note+LowE_type+'.csv','a') as chi_file: 
        ...
        xs.Plot.device = '/xs'    
        xs.Plot("data delchi")
        xs.Plot.device = out_dir+prefix+model_note+LowE_type+'_'+key+'_delchi.ps/cps' # save xspec version of plots
        xs.Plot("data delchi") 
        xs.Plot.device = '/null'
