...
v102: same as 101, add second high temp apec (absorbed) for hot halo 
...
for key in key_list:
    ...
    for i in range(len(spec_list)): #fit model to each spectrum linked between detectors    
        ...
        if (i==0):    
            xray_par[i]={1:'0.097,-1',2:'1,-1',3:'0,-1',4:str(LHB_target)+',-1',\
                          5:str(nh_target)+',-1',\
                          6:'0.22,0.05,0.1,0.15,0.3,0.35',7:'0.3,-1',8:'0,-1',9:'0.2,0.2,0.,0.1,0.5,10.',\
            ...
            TTS_par[i]={1:'0.,-1',
                          2:'0.5,-1',3:'1.,-1',4:'0,-1',5:'0.1,0.1,0.,0.,0.5,10.',
    ...
    TTS_names={1:'TTS_nH',2:'TTS_kT1',6:'TTS_kT2',5:'TTS_N'}
    ...
    for i, j in itertools.product(range(len(spec_list)), range(xmodel[i].nParameters)):
        if not ((xmodel[i](j+1).frozen)or(len(xmodel[i](j+1).link)>0)):
            ...
            if (xmodel[i](j+1).index==2):  #halo kT
                ...
                xs.Fit.steppar('best Xray:'+str(parnum)+' 0.15 0.35 20')
