...
#pull TTS_N from v99c csv file
TTS_Narr={}
TTS_file = DictReader(open(out_dir+'HSWCX1v99c'+nH_type+bkg_type+'.csv','r')) #*******************Fitted version
for row in TTS_file:
    for column, value in row.iteritems():
        TTS_Narr.setdefault(column, []).append(value)
for key in TTS_Narr: 
  TTS_Narr[key] = np.array(TTS_Narr[key])
...
file_header='Target,LHB_N,'+nH_type.split('_')[1]+',TTS_T1,TTS_N' #all other headers not hard-coded
file_format='\n{0[0]},{0[1]:.6f},{0[2]:.6f},{0[3]:.3f},{0[4]:.6f}'
csv_num=5  #next number in []
...
for key in key_list:
    ...
    #search TTS_Narr dict for correct TTS_N, store
    idx = np.where(TTS_Narr['Target']==key)[0]  #one tail spectrum per MSWCX target
    TTS_N = float(TTS_Narr['TTS_N'][idx])   
    ...
    halo_arr=[key,LHB_target,nh_target,TTS_T1,TTS_N]
    ...
    for i in range(len(spec_list)): #fit model to each spectrum linked between detectors    
        ...
        if (i==0):    
            ...
            SWCX_par[i]={1:line_ints['Energy']['LowE']+',-1',2:'0.001,-1',3:'0.,-1',\
                          4:line_ints['Energy']['O VIIa']+',-1',5:'0.001,-1',6:'0.015,0.01,0.,0.,0.06,10.',\
                          7:line_ints['Energy']['O VIIb']+',-1',8:'0.001,-1',9:'=SWCX:p6*'+OVII_ratio,\
                          10:line_ints['Energy']['O VIIIa']+',-1',11:'0.001,-1',12:'0.015,0.01,0.,0.,0.06,10.',\
                          13:line_ints['Energy']['O VIIIb']+',-1',14:'0.001,-1',15:'=SWCX:p12*'+OVIII_ratio,\
                          16:line_ints['Energy']['Ne IXa']+',-1',17:'0.001,-1',18:'0.015,0.01,0.,0.,0.06,10.',\
                          19:line_ints['Energy']['Ne IXb']+',-1',20:'0.001,-1',21:'=SWCX:p18*'+NeIX_ratio}
            TTS_par[i]={1:'0.,-1',
                          2:str(TTS_T1)+',-1',3:'1.,-1',4:'0,-1',5:str(TTS_N)+',-1',
    ...
    xray_names={2:'Halo_kT',5:'Halo_N'}  
    SWCX_names={3:'LowE',6:'O_VII',12:'O_VIII',18:'Ne_IX'}
    TTS_names={1:'TTS_nH',2:'TTS_T1',5:'TTS_N'}
    ...
    for i, j in itertools.product(range(len(spec_list)), range(smodel[i].nParameters)):
        if not ((smodel[i](j+1).frozen)or(len(smodel[i](j+1).link)>0)):
            ...
            xs.Fit.steppar('best SWCX:'+str(parnum)+' 0. 0.03 15') #up to ~0.86 LU
    ...
    for i, j in itertools.product(range(len(spec_list)), range(tmodel[i].nParameters)):
        if not ((tmodel[i](j+1).frozen)or(len(tmodel[i](j+1).link)>0)):
            ...
            if (tmodel[i](j+1).index==1):
                ...
                print 'steppar: TTS', parnum
                xs.Fit.steppar('best TTS:'+str(parnum)+' 0.5 1.0 15')  # low T
