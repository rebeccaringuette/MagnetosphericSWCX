...
out_dir=spec_dir
prefix = 'HSWCX1v128h2'   #h0: OVIIscale=1, h1: OVIIscale=0.5, h2: OVIIscale=1.5
OVIIscale='1.5'

#halo results
#halo_kT1, halo_N1 = 0.248, 0.834  #OVIIscale=1
#halo_kT2, halo_N2 = 0.973, 0.240  
#halo_kT1, halo_N1 = 0.213, 1.17  #OVIIscale=0.5
#halo_kT2, halo_N2 = 0.940, 0.261  #
halo_kT1, halo_N1 = 0.289, 0.595  #OVIIscale=1.5
halo_kT2, halo_N2 = 0.961, 0.230  #
nH_type = '_nHKip' #set type of nH used options: '_nHRT', '_nHHI4pi', '_nHSFDW', '_nHPZ'
model_note = nH_type+'_fake2'  #store choices
nh = 0.248
CXB_PI, CXB_N = 1.45, 0.382
LHB_T, LHB_N = 0.097, 0.186515

#initialize output format
file_header='Target,'+nH_type.split('_')[1]+',OVIIScale' #all other headers not hard-coded
file_format='\n{0[0]},{0[1]:.3f},{0[2]:.3f}'
...
key_list = ['Oct2018b+','Oct2019','Nov2019+','Nov2019','Dec2019+']  #simulated list only
...
for key in key_list:
    ...
    halo_arr=[key,nh,float(OVIIscale)]
    ...
    for i in range(len(spec_list)): #fit model to each spectrum linked between detectors    
        ...
        if (i==0):    
            xray_par[i]={1:str(nh)+',-1',\
                          2:str(halo_kT1)+',-1',3:'0.3,-1',4:'0,-1',5:str(halo_N1)+',-1',\
                          6:str(halo_kT2)+',-1',7:'0.3,-1',8:'0,-1',9:str(halo_N2)+',-1',\
    ...
    xray_model=xs.Model('tbabs*(apec+apec+pow)+apec','Xray',1)   #LHB+halo+CXB
