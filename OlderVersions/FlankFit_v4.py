...
model_note = 'MSWCXFlanks_v4T10a'+nH_type  #store choices, fit type
...
halo_pars={}
halo_file = DictReader(open(tail_dir+'MSWCXTails_v10a'+nH_type+'.csv','r')) #*******************Tail version
...
#make list of flank observations for fitted tail observations in chosen csv 
tail_list, flank_list = halo_pars['Target'], []  #get list of tail obs
for tail in tail_list:
    idx = np.where((SWCX_LU['ObsType']=='flank') & (SWCX_LU['MSWCX#']==tail))[0]  
    for i in idx: flank_list.append(SWCX_LU['MSWCX#'][i]+'_'+SWCX_LU['ObsName'][i]) 

#define SWCX energies
line_ints = {}
