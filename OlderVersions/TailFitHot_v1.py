...
TailFitHot_v1: add a second apec.
"""
...
model_note = 'MSWCXTailHot_v1'+nH_type  #store choices, fit type
...
for source_name in source_list:
    ...
    #choose combined tail observation from SWCX list
    idx = np.where((SWCX_LU['MSWCX#']==source_name) & (SWCX_LU['ObsType']=='tail'))[0]
    if len(idx)>1:  #select combined tail observation when more than one tail obs available
        for i in idx:   #find observation name with more than one tail, each are two characters
            if len(SWCX_LU['Obs#s'][i])>2: #if combined tail obs, pull spectra
                spec_list=np.sort(glob.glob(spec_dir+source_name+'_'+SWCX_LU['ObsName'][i]+'*_rebin40.pi')) #spectra order is 14, 38, 54
                idx = [i]  #save position for SWCX retreival
            if len(idx)==1: break  #exit loop if combined observation found               
    else:  #if only one tail obs available, use it     
        spec_list=np.sort(glob.glob(spec_dir+source_name+'_'+SWCX_LU['ObsName'][idx][0]+'*_rebin40.pi'))      
    
    #retrieve SWCX_LU values for current combined tail obs   
    if SWCX_LU['OVII'][idx][0]=='': continue   #skip if no SWCX calculations available
    ...
    for i in range(len(spec_list)): #fit model to each spectrum linked between detectors        
        ...
        if (i==0):    
            xray_par[i]={1:'0.097,-1',2:'1,-1',3:'0,-1',4:LHB_target+',-1',\
                          5:nh_target+',-1',\
                          6:'0.2,-1',7:'0.3,-1',8:'0,-1',9:'1.0,0.1,0,0.1,1e20,1e24',\
                          10:'1.45,-1',11:'0.38,-1',\
