...
#set up folders 
source_list=np.array(('MSWCX9','MSWCX10','MSWCX11','MSWCX12'),dtype=str) #done: 'MSWCX1','MSWCX2','MSWCX3','MSWCX4','MSWCX5',  #Need SWCX: 'MSWCX6','MSWCX7','MSWCX8',
ID_list = np.array((76,77,78,79),dtype=int) #done: 68,69,70,71,72,75,    #Need SWCX: 73,74
spec_dir='/home/ringuette/halosat_sourceanalysis/MSWCX_2020Analysis/Rebinned_files_ind/' #directory where all spectra are stored
code_dir='/home/ringuette/halosat_sourceanalysis/AnalysisCode/MSWCXCode/'
out_dir ='/home/ringuette/halosat_sourceanalysis/MSWCX_2020Analysis/IndOut/'
...
model_note = 'MSWCXIndHot_v1T13net'+nH_type  #store choices, fit type

#initialize output format
file_header='ObsName,RA,DEC,GLon,Glat,LHB_N,'+nH_type.split('_')[1]+',halo_kT1,halo_N1,halo_kT2,halo_N2' #all other headers not hard-coded
file_format='\n{0[0]},{0[1]:.6f},{0[2]:.6f},{0[3]:.6f},{0[4]:.6f},{0[5]:.6f},{0[6]:.6f},{0[7]:.6f},{0[8]:.6f},{0[9]:.6f},{0[10]:.6f}'
csv_num=11  #next number in []
...
halo_pars={}
halo_file = DictReader(open(code_dir+'MSWCXTails_v13v3_halopars'+nH_type+'.csv','r')) #*******************Tail version
...
#make net list of individual observations per target
ind_list=[]
for source_name in source_list:  
    spec_list=np.sort(glob.glob(spec_dir+source_name+'_*_cl1_d14_rebin40.pi')) #spectra order is 14, 38, 54
    for spec in spec_list: ind_list.append(source_name+'_'+spec.split('/')[-1].split('_')[1])
    
#collect average SWCX line ratios 
OVII_ratio = '0.126' #str('{:.3f}'.format(float(line_ints['Average']['O VIIb'])/float(line_ints['Average']['O VIIa'])))
OVIII_ratio = '0.320' #str('{:.3f}'.format(float(line_ints['Average']['O VIIIb'])/float(line_ints['Average']['O VIIIa'])))
NeIX_ratio = '0.100' #str('{:.3f}'.format(float(line_ints['Average']['Ne IXb'])/float(line_ints['Average']['Ne IXa'])))
LowE_ratio = '0.966'   #100% H = 0.966, 100% He = 0.925, 50% each = 0.946, not sensitive so chose half

#loop through MSWCX#s (one tail obs per target)
for source_name in ind_list:
    ...
    idx = np.where(halo_pars['\xef\xbb\xbfTarget']==source_name.split('_')[0])[0]  #one tail spectrum per MSWCX target
    halo_kT1, halo_N1 = float(halo_pars['halo_kT1'][idx][0]), float(halo_pars['halo_N1'][idx][0])
    halo_kT2, halo_N2 = float(halo_pars['halo_kT2'][idx][0]), float(halo_pars['halo_N2'][idx][0])
    ...
    halo_arr=[source_name,RA[source_idx][0],DEC[source_idx][0],glon[source_idx][0],glat[source_idx][0],LHB[source_idx][0],nh[source_idx][0],halo_kT1,halo_N1,halo_kT2,halo_N2]
    ...
    for i in range(len(spec_list)): #fit model to each spectrum linked between detectors        
        ...
        if (i==0):    
            ...
            xray_par[i]={1:'0.097,-1',2:'1,-1',3:'0,-1',4:LHB_target+',-1',\
                          ...
                          10:'1.45,-1',11:'0.38,-1',\
                          12:'=Xray:p12',13:'=Xray:p13',14:'0,-1',15:'=Xray:p15'}      
    ...
    for i, j in itertools.product(range(len(spec_list)), range(smodel[i].nParameters)):
        if not ((smodel[i](j+1).frozen)or(len(smodel[i](j+1).link)>0)):
            ...
            if source_name==ind_list[0]:  #first key
    ...
    if source_name==ind_list[0]: #initialize csv file header
