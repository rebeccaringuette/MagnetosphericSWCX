...
TailFitHot_v1: add a second apec., kT1= 0.2
v2: only on selected tails, kt1 = free
v3: v2, on clean combined tail obs
v4: Fix kT1 to 0.2086, the weighted average of the best kT1s from prev best fits (including hot)

v5: update code to work with new HSWCX calcs from Dimitra, other as v3
v6: same as v5, loop through SWCX factors
"""
...
source_list=np.array(['MSWCX6'],dtype=str) #mod2 done: 'MSWCX4'
# need HSWCX calcs for: 'MSWCX1','MSWCX2','MSWCX3','MSWCX11','MSWCX12'
ID_list = np.array([73],dtype=int)  #mod2 done: 71
...
mod_choice = '2'
model_note = 'MSWCXTailsHot_v6'+nH_type+'_mod'+mod_choice+'f'  #store choices, fit type

#set SWCX scaling factors
'''# Model 2 lists
SWCX_scale = {'MSWCX4':np.array(['0.50','0.75','1.00','1.25','1.50'],dtype=str),\
              'MSWCX6':np.array(['0.50','0.75','1.00','1.25','1.50'],dtype=str)}  #a_list

SWCX_scale = {'MSWCX4':np.array(['0.80','0.90','1.10','1.20'],dtype=str),\
              'MSWCX6':np.array(['0.20','0.30','0.40'],dtype=str)}  #b_list

SWCX_scale = {'MSWCX4':np.array(['0.85','0.86','0.87','0.88','0.89','0.91','0.92','0.93','0.94'],dtype=str),\
              'MSWCX6':np.array(['0.00','0.10'],dtype=str)}  #c_list
         
SWCX_scale = {'MSWCX4':np.array(['0.00','0.25','0.95','0.96','0.97','0.98','0.99','1.01','1.02','1.03',\
                                  '1.04','2.00','2.50','3.00','3.50'],dtype=str),\
              'MSWCX6':np.array(['1.75','2.00','2.25','2.50'],dtype=str)}  #d_list

SWCX_scale = {'MSWCX4':np.array(['2.60','2.70','2.80','2.90'],dtype=str),\
              'MSWCX6':np.array(['1.80','1.90'],dtype=str)}  #e_list

SWCX_scale = {'MSWCX6':np.array(['0.01','0.05'],dtype=str)}  #f_list
'''
#Model 1 Lists    
...
for source_name in source_list:
    ...
    for OVII_Oratio in SWCX_scale[source_name]:        
        ...
        for i in range(len(spec_list)): #fit model to each spectrum linked between detectors        
            ...
            if (i==0):    
                xray_par[i]={1:'0.097,-1',2:'1,-1',3:'0,-1',4:LHB_target+',-1',\
                              5:nh_target+',-1',\
                              6:'0.22,0.02,0.1,0.15,0.3,0.35',7:'0.3,-1',8:'0.,-1',9:'1.0,0.1,0,0.1,1e20,1e24',\
                              10:'1.45,-1',11:'0.382,-1',\
                              12:'0.75,0.2,0.35,0.5,1.5,2.0',13:'0.3,-1',14:'0.,-1',15:'1.0,0.1,0,0.1,1e20,1e24'}
                ...
                xray_par[i]={1:'0.097,-1',2:'1,-1',3:'0,-1',4:LHB_target+',-1',\
                              ...
                              10:'1.45,-1',11:'0.382,-1',\
                              12:'=Xray:p12',13:'0.3,-1',14:'0.,-1',15:'=Xray:p15'}      
        ...
        xray_model=xs.Model('apec+phabs*(apec+pow+apec)','Xray',2)   #LHB+absorbed halo+absorbed CXB 
        ...
        for i, j in itertools.product(range(len(spec_list)), range(xmodel[i].nParameters)):
            if not ((xmodel[i](j+1).frozen)or(len(xmodel[i](j+1).link)>0)):
                ...
                if (xmodel[i](j+1).index==12):  #halo kT2
                    print 'steppar: Xray', parnum
                    xs.Fit.steppar('best Xray:'+str(parnum)+' 0.5 1.5 10')
                if (xmodel[i](j+1).index==15):  #halo N2
                    print 'steppar: Xray', parnum
                    xs.Fit.steppar('best Xray:'+str(parnum)+' 0. 2. 10')   
