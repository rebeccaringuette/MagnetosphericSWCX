...
source_list=np.array(('MSWCX4','MSWCX5','MSWCX6','MSWCX7','MSWCX8','MSWCX9','MSWCX10'),dtype=str) 
# need HSWCX calcs for: 'MSWCX1','MSWCX2','MSWCX3','MSWCX11','MSWCX12'
ID_list = np.array((71,72,73,74,75,76,77),dtype=int)  
...
mod_choice = '2'
model_note = 'MSWCXTailsHot_v5'+nH_type+'_mod'+mod_choice  #store choices, fit type
...
for source_name in source_list:
    ...
    OVII_N, OVIII_N, NeIX_N = float(SWCX_LU['OVII_'+mod_choice][idx][0])*0.035, float(SWCX_LU['OVIII_'+mod_choice][idx][0])*0.035, \
          float(SWCX_LU['NeIX_'+mod_choice][idx][0])*0.035
    #print spec_list[0].split('/')[-1], OVII_N, OVIII_N, NeIX_N
    ...
    for i, j in itertools.product(range(len(spec_list)), range(xmodel[i].nParameters)):
        if not ((xmodel[i](j+1).frozen)or(len(xmodel[i](j+1).link)>0)):
            ...
            if (xmodel[i](j+1).index==9):  #halo N1
                print 'steppar: Xray', parnum
